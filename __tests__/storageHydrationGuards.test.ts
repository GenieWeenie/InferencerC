...
describe('storage hydration guards', () => {
  ...
  test('sanitizes cloudsync/plugin/tag/help/apidocs/topic persistence mutation paths', async () => {
    let cloudSyncService: typeof import('../src/renderer/services/cloudSync').cloudSyncService;
    let pluginSystemService: typeof import('../src/renderer/services/pluginSystem').pluginSystemService;
    let autoTaggingService: typeof import('../src/renderer/services/autoTagging').autoTaggingService;
    let contextualHelpService: typeof import('../src/renderer/services/contextualHelp').contextualHelpService;
    let apiDocsGeneratorService: typeof import('../src/renderer/services/apiDocsGenerator').apiDocsGeneratorService;
    let topicModelingService: typeof import('../src/renderer/services/topicModeling').topicModelingService;
    ...
    jest.isolateModules(() => {
      ...
      ({ pluginSystemService } = require('../src/renderer/services/pluginSystem') as typeof import('../src/renderer/services/pluginSystem'));
      ({ autoTaggingService } = require('../src/renderer/services/autoTagging') as typeof import('../src/renderer/services/autoTagging'));
      ({ contextualHelpService } = require('../src/renderer/services/contextualHelp') as typeof import('../src/renderer/services/contextualHelp'));
      ...
      ({ topicModelingService } = require('../src/renderer/services/topicModeling') as typeof import('../src/renderer/services/topicModeling'));
      ({ HistoryService } = require('../src/renderer/services/history') as typeof import('../src/renderer/services/history'));
    });

    const cloudConfig = cloudSyncService.updateConfig({
      enabled: 'yes' as any,
      ...
      encryptionSalt: '   ',
      deviceId: '   ',
      syncSettings: 'true' as any,
      syncTemplates: 1 as any,
      selectedConversationIds: [' session-1 ', 'session-1', '  ', 2] as any,
    } as any);
    expect(cloudConfig.enabled).toBe(true);
    expect(cloudConfig.baseUrl).toBe('https://sync.example.com');
    expect(cloudConfig.token).toBe('token-1');
    expect(cloudConfig.accountId).toBeUndefined();
    expect(cloudConfig.email).toBe('user@example.com');
    expect(cloudConfig.encryptionSalt).toBeUndefined();
    expect(cloudConfig.deviceId).toMatch(/.+/);
    expect(cloudConfig.syncSettings).toBe(true);
    expect(cloudConfig.syncTemplates).toBe(true);
    expect(cloudConfig.selectedConversationIds).toEqual(['session-1']);

    const installedPlugin = await pluginSystemService.installPlugin({
      id: 'safe.plugin',
      name: ' Safe Plugin ',
      version: ' 1.0.0 ',
    ...
    expect(installedPlugin.manifest.id).toBe('safe.plugin');
    (pluginSystemService as any).plugins.set('invalid-plugin', {
      manifest: {
        id: '',
        name: 'Bad',
        version: '1.0.0',
    ...
    (pluginSystemService as any).plugins.set('duplicate-plugin', {
      manifest: { ...installedPlugin.manifest, name: ' Duplicate Name ' },
      enabled: false,
      installedAt: 1,
    });
    (pluginSystemService as any).savePlugins();
    const persistedPlugins = JSON.parse(localStorage.getItem('plugins') ?? '[]');
    expect(persistedPlugins).toHaveLength(1);
    expect(persistedPlugins[0]?.manifest?.id).toBe('safe.plugin');
    expect(persistedPlugins[0]?.manifest?.name).toBe('Safe Plugin');

    autoTaggingService.addCustomTag({
      id: ' tag-1 ',
      name: ' Tag One ',
      color: ' #fff ',
      category: ' custom ',
    });
    autoTaggingService.addCustomTag({ id: '   ', name: 'Bad' } as any);
    expect(autoTaggingService.getCustomTags()).toEqual([
      { id: 'tag-1', name: 'Tag One', color: '#fff', category: 'custom' },
    ]);
    autoTaggingService.updateTags(' session-1 ', [' alpha ', 'alpha', ' ', 7] as any, 'yes' as any);
    expect(autoTaggingService.getTags('session-1')).toEqual({
      sessionId: 'session-1',
      tags: ['alpha'],
      autoGenerated: true,
      taggedAt: expect.any(Number),
    });
    expect(autoTaggingService.getConversationsByTag(' alpha ')).toEqual(['session-1']);

    contextualHelpService.setTooltip({
      id: ' tip-1 ',
      target: ' #target ',
      title: ' Title ',
      content: ' Content ',
    ...
    contextualHelpService.setTooltip({ id: '   ', target: '#x', title: 'Bad', content: 'Bad' } as any);
    expect(contextualHelpService.getTooltip(' tip-1 ')).toEqual({
      id: 'tip-1',
      target: '#target',
      title: 'Title',
      content: 'Content',
    ...
    contextualHelpService.removeTooltip('   ');
    const persistedHelp = JSON.parse(localStorage.getItem('contextual_help') ?? '[]');
    expect(persistedHelp.some((entry: any) => entry.id === 'tip-1')).toBe(true);

    HistoryService.saveSession({
      id: 'session-api-mutation',
      title: 'API Session',
      lastModified: Date.now(),
      modelId: 'model-1',
      messages: [{ role: 'user', content: 'Generate docs' }],
    });
    const generatedDocs = await apiDocsGeneratorService.generateDocs('session-api-mutation', async () => ({
      content: JSON.stringify({
        title: ' API Docs ',
        description: ' Docs description ',
        endpoints: [
          { method: 'GET', path: '/status', description: 'Status', responses: [{ statusCode: 200, description: 'OK' }] },
          { method: 'get', path: '/status', description: 'Duplicate', responses: [{ statusCode: 200, description: 'OK' }] },
        ],
        models: [
          { name: 'Status', description: 'status model', properties: { ok: { type: 'boolean', description: 'ok' } } },
          { name: 'status', description: 'duplicate model', properties: { ok: { type: 'boolean', description: 'ok' } } },
        ],
        examples: [
          { title: 'Example', request: { method: 'GET', url: '/status' }, response: { status: 200, body: { ok: true } } },
          { title: 'Example', request: { method: 'GET', url: '/status' }, response: { status: 200, body: { ok: true } } },
        ],
      }),
    }));
    expect(generatedDocs).not.toBeNull();
    expect(generatedDocs?.endpoints).toHaveLength(1);
    expect(generatedDocs?.models).toHaveLength(1);
    expect(generatedDocs?.examples).toHaveLength(1);
    expect(apiDocsGeneratorService.getDocumentation(' session-api-mutation ')?.title).toBe('API Docs');

    (topicModelingService as any).saveTopicModel({
      sessionId: ' session-topic-1 ',
      topics: [
        {
          id: ' topic-1 ',
          name: ' Topic One ',
          keywords: [' alpha ', 'alpha', 2],
          weight: 2,
          messageIndices: [0, 0, -1, 1],
          frequency: 5.9,
        },
        {
          id: 'topic-1',
      ...
      topicDistribution: { ' topic-1 ': 2, bad: 'x' },
      analyzedAt: 9.9,
    });
    const topicModel = topicModelingService.getTopicModel(' session-topic-1 ');
    expect(topicModel).toMatchObject({
      sessionId: 'session-topic-1',
      analyzedAt: 9,
    });
    expect(topicModel?.topics).toHaveLength(1);
    expect(topicModel?.topics[0]).toMatchObject({
      id: 'topic-1',
      name: 'Topic One',
      keywords: ['alpha'],
      weight: 1,
    ...
    expect(JSON.parse(localStorage.getItem('topic_models') ?? '[]')).toHaveLength(1);
  });
