...
import { autoTaggingService, type ConversationTags } from '../src/renderer/services/autoTagging';
...
describe('SearchService.searchAsync', () => {
  ...
  const getTagsMock = autoTaggingService.getTags as jest.MockedFunction<typeof autoTaggingService.getTags>;
  ...
  it('uses one bulk tag lookup for tag-heavy async searches', async () => {
    const sessionCount = 200;
    const targetTag = 'focus-tag';
    const sessions: ChatSession[] = [];
    const byId = new Map<string, ChatSession>();
    const tagsLookup = new Map<string, ConversationTags>();
    const searchableIds = new Set<string>();

    for (let i = 0; i < sessionCount; i++) {
      const session = createSession(`s${i}`, `session-${i}`, `alpha beta gamma ${i}`);
      sessions.push(session);
      byId.set(session.id, session);
      searchableIds.add(session.id);
      if (i % 10 === 0) {
        tagsLookup.set(session.id, {
          sessionId: session.id,
          tags: [targetTag, 'secondary-tag'],
          autoGenerated: false,
          taggedAt: 1,
        });
      } else {
    ...
    getAllSessionsMock.mockReturnValue(sessions);
    getTagsLookupMock.mockReturnValue(tagsLookup);
    searchSessionsMock.mockReturnValue(searchableIds);
    getSessionMock.mockImplementation((id: string) => byId.get(id) || null);
    workerSearchMock.mockResolvedValue({
      results: [],
      stats: {
    ...
    await SearchService.searchAsync('alpha beta gamma', { tags: [targetTag] });
